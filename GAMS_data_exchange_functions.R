...
RunFoodOpt <- function(scenario="tc_min", cluster="reg-urb-inc", nutri) { #"reg-urb" or "reg-urb-inc"
  ...
  for (i in clsnames) {
    ...
    wgdx(paste0(interim_path, "DLE_data.gdx"), f, fg, a, pr, c, e, r, grp, nn, ig)  # Not sure how I can use gdx with diff names in the .gms file
    # Sys.sleep(1)
    params <- paste0('DNLP=COUENNE LP=CPLEX Gdx=', interim_path, 'DLE_output_', i, ' LogOption=2 Logfile=', interim_path, 'DLE_log_', i, '.log //scenario=', scenario) #--INPUT=DLE_data.gdx 
    ...
    result$obj <- rgdx(paste0(interim_path, "DLE_output_", i, ".gdx"), list(name="dvcon"))
    result$x <- rgdx(paste0(interim_path, "DLE_output_", i, ".gdx"), list(name="x"))
    result$base_tc <- rgdx(paste0(interim_path, "DLE_output_", i, ".gdx"), list(name="report_baseline_cost"))
    result$result_tc <- rgdx(paste0(interim_path, "DLE_output_", i, ".gdx"), list(name="report_result_cost"))
    result$base_tem <- rgdx(paste0(interim_path, "DLE_output_", i, ".gdx"), list(name="report_baseline_emission"))
    result$result_tem <- rgdx(paste0(interim_path, "DLE_output_", i, ".gdx"), list(name="report_result_emission"))
    result$base_nutri <- rgdx(paste0(interim_path, "DLE_output_", i, ".gdx"), list(name="report_baseline_nutri"))
    result$result_nutri <- rgdx(paste0(interim_path, "DLE_output_", i, ".gdx"), list(name="report_result_nutri"))
    result$base_cal <- rgdx(paste0(interim_path, "DLE_output_", i, ".gdx"), list(name="report_baseline_cal"))
    result$result_cal <- rgdx(paste0(interim_path, "DLE_output_", i, ".gdx"), list(name="report_result_cal"))
    result$base_kg <- rgdx(paste0(interim_path, "DLE_output_", i, ".gdx"), list(name="report_baseline_kg"))
    result$result_kg <- rgdx(paste0(interim_path, "DLE_output_", i, ".gdx"), list(name="report_result_kg"))
    result$infeasible <- sum(rgdx(paste0(interim_path, "DLE_output_", i, ".gdx"), list(name="s_cost_fix"))$val[,3]) +
      sum(rgdx(paste0(interim_path, "DLE_output_", i, ".gdx"), list(name="s_cal"))$val[,3]) +
      sum(rgdx(paste0(interim_path, "DLE_output_", i, ".gdx"), list(name="s_nutri"))$val[,3]) +
      sum(rgdx(paste0(interim_path, "DLE_output_", i, ".gdx"), list(name="s_maxcap"))$val[,3]) +
      sum(rgdx(paste0(interim_path, "DLE_output_", i, ".gdx"), list(name="s_mincap"))$val[,3])
...
wat <- cbind(1:dim(items_to_optimize)[1], runif(dim(items_to_optimize)[1]))
  
# Write GDX for one case (mainly for debugging) - for testing water 
WriteOptGDX_w <- function(scenario="tc_min", cluster="reg-urb-inc", zone="E0_1", nutri) { #"reg-urb" or "reg-urb-inc"
  ...
  e  <- list(name='e',  type='parameter', dim=1, form='sparse', uels=food_items, 
             val= e, 
             ts="non-CO2e emission factor of food f (kgCO2e per kg)", domains="f")
  w  <- list(name='w',  type='parameter', dim=1, form='sparse', uels=food_items, 
             val= wat, 
             ts="water usage factor of food f (l per kg)", domains="f")
  ...
  wgdx(paste0(mcma_path, "data/DLE_data.gdx"), f, fg, a, pr, c, e, w, r, grp, nn, ig)  # Not sure how I can use gdx with diff names in the .gms file
}
...
OrganizeOptOutputs <- function(scenariosOpt) {
  ...
  for(i in 1:l) {
    ...
    # only for MA now
    xlsx::write.xlsx(ArrConsumption(l_consum[[i]], "MA"), paste0(gamsdir, "Results/Consumption_outputs-", runname, ".xlsx"), names(scenariosOpt)[i], 
                     row.names = FALSE, col.names = TRUE, append = ifelse(i==1, FALSE, TRUE))
    xlsx::write.xlsx(l_param[[i]], paste0(gamsdir, "Results/Parameter_outputs-", runname, ".xlsx"), names(scenariosOpt)[i], 
                     row.names = TRUE, col.names = TRUE, append = ifelse(i==1, FALSE, TRUE))
    xlsx::write.xlsx(t(l_nutr[[i]]), paste0(gamsdir, "Results/Nutrient_outputs-", runname, ".xlsx"), names(scenariosOpt)[i], 
                     row.names = TRUE, col.names = TRUE, append = ifelse(i==1, FALSE, TRUE))
    xlsx::write.xlsx(l_kg[[i]], paste0(gamsdir, "Results/Kg_outputs-", runname, ".xlsx"), names(scenariosOpt)[i],
                     row.names = FALSE, col.names = TRUE, append = ifelse(i==1, FALSE, TRUE))
  ...
  for(j in 1:ll) {
    ...
    xlsx::write.xlsx(conbycls, paste0(gamsdir, "Results/Consumption_outputs-", runname, ".xlsx"), 
